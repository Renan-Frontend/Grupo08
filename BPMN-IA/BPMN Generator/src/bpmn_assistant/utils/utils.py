...
def is_ollama_model(model: str) -> bool:
    # Convenção: "ollama/<nome-do-modelo>"
    return model.startswith("ollama/")


def normalize_ollama_model(model: str) -> str:
    # Remove o prefixo "ollama/" para mandar só o nome do modelo ao Ollama
...
def get_llm_facade(
    ...
    # ✅ OLLAMA: não exige API key e não depende de enums
    ...
    # Providers existentes (mantém comportamento atual)
...
def get_available_providers(api_keys: dict[str, str] | None = None) -> dict:
    ...
    # Checa Ollama (sem depender de key)
    load_dotenv(override=True)
    ollama_base = (os.getenv("OLLAMA_BASE_URL") or "http://host.docker.internal:11434").rstrip("/")
