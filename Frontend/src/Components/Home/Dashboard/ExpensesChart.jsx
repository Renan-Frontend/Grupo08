    ...
    <div className={styles.tooltip}>
      ...
      {payload.map((entry) => (
        <p key={entry.dataKey} className={styles.tooltipRow}>
          ...
          {entry.name}: <strong>{fmt ? fmt(entry.value) : formatBRL(entry.value)}</strong>
        </p>
      ))}
    </div>
  );
...
const ExpensesChart = ({ data, onEditData, meta, metaType = 'value', onMetaChange, onMetaTypeChange }) => {
  const allVals = (data || []).flatMap(d => [d.fixas, d.variaveis].filter(v => v != null));
    ...
    <div className={styles.chartCard}>
      ...
      <ResponsiveContainer width="100%" height={260}>
        ...
        >
          ...
          <XAxis dataKey="mes" tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} interval={0} />
          <YAxis tickFormatter={isPct ? (v) => `${((v - minVal) / range * 100).toFixed(0)}%` : formatBRL} tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} />
