...
const CustomTooltip = ({ active, payload, label, fmt }) => {
  ...
  return (
    <div className={styles.tooltip}>
      ...
      <p className={styles.tooltipValue}>{fmt ? fmt(v) : v.toFixed(1)}</p>
    </div>
  );
};

const ConversionsChart = ({ data, onEditData, meta, metaType = 'value', onMetaChange, onMetaTypeChange }) => {
  const vals = (data || []).map(d => d.taxa).filter(v => v != null);
  const minVal = vals.length ? Math.min(...vals) : 0;
  const maxVal = vals.length ? Math.max(...vals) : 0;
  const media = Number(((minVal + maxVal) / 2).toFixed(2));
