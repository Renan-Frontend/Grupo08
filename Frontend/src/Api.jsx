...
export const REGISTROS_GET = (token, papelNegocio = "", entidadeId = "") => {
  const params = new URLSearchParams();
  if (papelNegocio) params.set("papelNegocio", papelNegocio);
  if (entidadeId) params.set("entidadeId", entidadeId);
  const qs = params.toString();
  return createRequest(`/registros${qs ? `?${qs}` : ""}`, "GET", null, token);
};

export const REGISTROS_POST = (body, token) =>
  createRequest("/registros", "POST", body, token);

export const REGISTROS_PUT = (id, body, token) =>
  createRequest(`/registros/${id}`, "PUT", body, token);

export const REGISTROS_DELETE = (id, token) =>
  createRequest(`/registros/${id}`, "DELETE", null, token);
// Usuário autenticado
...
export const DOCUMENTOS_LIST = (token, owner = "", opportunityId = null) => {
  const params = new URLSearchParams();
...
export const WORKFLOW_INSTANCES = (opId, token, status = null) =>
  createRequest(
    `/workflow/${opId}/instances${status ? `?status=${encodeURIComponent(status)}` : ""}`,
    "GET",
    null,
    token,
  );
...
export const WORKFLOW_INSTANCE_DETAIL = (opId, instanceId, token) =>
  createRequest(
    `/workflow/${opId}/instances/${encodeURIComponent(instanceId)}`,
    "GET",
    null,
    token,
  );
...
export const OPORTUNIDADES_LIST = (
  token,
  { page = 1, limit = 100, search = "", owner = "", shared = "" } = {},
) => {
  const params = new URLSearchParams();
...
export const WORKFLOW_TASKS_LIST = (
  token,
  {
    status,
    assignee,
...
export const BPMN_VERSION_GET = (opId, version, token) =>
  createRequest(
    `/oportunidades/${opId}/versions/${version}`,
    "GET",
    null,
    token,
  );
...
export const BPMN_VERSION_RESTORE = (opId, version, token) =>
  createRequest(
    `/oportunidades/${opId}/versions/${version}/restore`,
    "POST",
    {},
    token,
  );
...
export const WEBHOOK_DELIVERIES = (webhookId, params, token) => {
  ...
  const q = qs.toString();
  return createRequest(
    `/webhooks/${webhookId}/deliveries${q ? `?${q}` : ""}`,
    "GET",
    null,
    token,
  );
